...
\title{Beyond Accuracy: A Feature-Node Graph Representation for
Interpretable, Deployable and Transportable Cardiovascular Risk
Prediction}
...
Random Forest, $p=1.00$ against XGBoost, $p=0.66$ against LightGBM) on
303 patients; we report this parity as a finding, not an improvement. The formulation differs
...
collapses to a single-class predictor on one external cohort. Fitting
every component on Cleveland and touching each external cohort once,
the model attains higher ROC-AUC in all six model--cohort comparisons
...
\ifnum\ACCESSCLASS=1
  \begin{keywords}
  Cardiovascular disease, clinical tabular data, distribution shift,
  explainable artificial intelligence, external validation, feature
...
information along clinically meaningful feature relationships, and the
...
vs.\ $0.6241$; all Holm-adjusted $p=1.0$, Tables~\ref{tab:msttau}
and~\ref{tab:msttaustats}).
...
reported in Table~\ref{tab:pooled} and agree with the fold-averaged
values to within $0.003$ on every metric.
...
Table~\ref{tab:stats} reports the significance tests, now extended to
all four non-neural baselines. None of the eight tests (four
...
Regression, $p=0.77$ against Random Forest, $p=1.00$ against XGBoost and
$p=0.66$ against LightGBM, and the Wilcoxon signed-rank test on per-fold
F1 gives $p=0.25$, $p=0.51$, $p=0.60$ and $p=0.15$, respectively (Holm-adjusted
$p\geq0.61$ throughout). We therefore characterise the GCN as
\emph{statistically indistinguishable from} strong classical
baselines---including modern gradient-boosted trees---not as a decisive
improvement. This honest positioning is, in our view, more credible than
an inflated accuracy claim and redirects the contribution toward
...
margin is modest. Per-seed DeLong tests on the pooled out-of-fold
...
reports Brier score and expected calibration error. Out-of-fold on
...
therefore computed decision curves, reporting net benefit
...
(Table~\ref{tab:convs}). No operator differs significantly from GCN:
DeLong $p\ge0.63$ and McNemar $p\ge0.057$ in every comparison
(Table~\ref{tab:convstats}). GIN attains a nominally higher accuracy
...
\label{sec:threshold}
Table~\ref{tab:threshold} reports performance
...
correction over nine paired tests (Tables~\ref{tab:toposigperf}
and~\ref{tab:toposig}), replacing the
...
choice of operator, and a clinically meaningful topology is measurably
...
(Table~\ref{tab:nodeidsweep}). The sign reversal of the graph gap holds at
...
(Table~\ref{tab:nodeidgapsweep}); none of the twelve
...
\texttt{ca} and \texttt{thal}, in agreement with the clinical literature.

Crucially, GNNExplainer and Integrated Gradients agree strongly
(Table~\ref{tab:xai_agree}; Spearman
$\rho=0.79$, top-$k$ Jaccard $0.56$). Because these two
...
external validation of the full model is impossible in principle, not
...
\caption{External validation. Left: ROC-AUC by cohort for the transported
...
Gradients: Spearman $\rho=0.79$, top-$k$ Jaccard $0.56$) that align with
...
only a strictly shared protocol with significance testing distinguishes
genuine improvement from favourable splitting, and reporting a model's
non-superiority is often the more informative result.
